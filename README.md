...
### 環境変数の設定 ( .env )
このシステムでは、セキュリティと環境の柔軟性のために環境設定を `.env` ファイルで管理しています。
初回実行前に `.env` ファイルを確認し、必要に応じて設定を変更してください。

主要な設定項目：
- `DB_PATH`: データベースファイルの保存先（デフォルト: `/data/gyokaku.db`）
- `SECRET_KEY`: Flask用のランダムなシークレットキー
- `OLLAMA_BASE_URL`: AI連携先（Ollama）のURL
